...
describe("orchestration", () => {
  ...
  it("sorts unpinned tabs after the pinned block", async () => {
    ...
    expect(mocks.applyOrder).toHaveBeenCalledWith([9, 1, 2]);
  });
  ...
  it("keeps the pinned tab in its front region when pinned are not ignored", async () => {
    mocks.getPrefs.mockResolvedValue({ ...prefs, ignorePinned: false });

    await expect(runSort("domain")).resolves.toEqual({ moved: 2 });

    expect(mocks.applyOrder).toHaveBeenCalledWith([9, 1, 2]);
  });

  it("sorts pinned tabs within their own region without interleaving unpinned", async () => {
    const pinnedTabs: TabLite[] = [
      { id: 1, title: "Zed", url: "https://zed.example", index: 0, pinned: true },
      { id: 2, title: "Ace", url: "https://ace.example", index: 1, pinned: true },
      { id: 3, title: "Mid", url: "https://mid.example", index: 2, pinned: false },
    ];
    mocks.getCurrentWindowTabs.mockResolvedValue(pinnedTabs);
    mocks.getPrefs.mockResolvedValue({ ...prefs, ignorePinned: false });

    await expect(runSort("title")).resolves.toEqual({ moved: 2 });

    expect(mocks.applyOrder).toHaveBeenCalledWith([2, 1, 3]);
  });
