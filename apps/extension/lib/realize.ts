...
import type { GroupColor, GroupSpec, TabPlan } from "./types";
...
async function runGroups(
  windowId: number,
  groups: GroupSpec[],
): Promise<{ grouped: number; createdGroups: Array<{ title: string; color: GroupColor }> }> {
  ...
  // A "create" op carries only { key, tabIds } — title/color land in the trailing
  // "update" op — so recover each created group's metadata from `desired` by key
  // (unique and plan-ordered), reporting exactly the groups this run created.
  const createdGroups = desired
    .filter((group) => createdGroupIds.has(group.key))
    .map(({ title, color }) => ({ title, color }));

  return { grouped, createdGroups };
}
...
export async function applyPlan(
  ...
  const { grouped, createdGroups } = await runGroups(windowId, plan.groups);
  ...
  return { grouped, createdGroups, closed };
}
