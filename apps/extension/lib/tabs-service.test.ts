...
describe("tabs service", () => {
  ...
  it("re-queries before applying order and skips stale ids", async () => {
    ...
    await applyOrder([3, 2, 1], { afterPinned: 2 });

    expect(move).toHaveBeenNthCalledWith(1, 3, { index: 2 });
    expect(move).toHaveBeenNthCalledWith(2, 1, { index: 3 });
    expect(move).toHaveBeenCalledTimes(2);
  });

  it("does not move empty or single-id order lists", async () => {
    await applyOrder([], { afterPinned: 0 });
    await applyOrder([1], { afterPinned: 0 });
