...
describe("applyPlan — group-aware realize", () => {
  ...
  it("drops a fully-vanished GroupSpec and filters a partially-vanished one to its survivors", async () => {
    ...
    // Only "partial" retained a survivor to create; "gone" dropped out entirely.
    expect(result).toEqual({
      grouped: 1,
      groupsCreated: 1,
      createdGroupKeys: ["partial"],
      closed: 0,
      vanished: 2,
    });
  ...
  it("reports only the freshly-created group's key when a larger pre-existing group is left untouched", async () => {
    // The refuter's scenario: a 6-tab github group already exists; tidy (with
    // regroupExisting off) leaves it an atomic block and creates ONE new group
    // for the 2 fresh stackoverflow tabs. createdGroupKeys must name the group
    // that was actually created (stackoverflow.com), NOT the larger github one a
    // count-desc re-derivation over the post-mutation domain groups would pick.
    const fake = createFakeBrowser(
      [
        { id: 1, pinned: false, groupId: 42 },
        { id: 2, pinned: false, groupId: 42 },
        { id: 3, pinned: false, groupId: 42 },
        { id: 4, pinned: false, groupId: 42 },
        { id: 5, pinned: false, groupId: 42 },
        { id: 6, pinned: false, groupId: 42 },
        { id: 7, pinned: false, groupId: TAB_GROUP_NONE },
        { id: 8, pinned: false, groupId: TAB_GROUP_NONE },
      ],
      [{ id: 42, title: "github.com", color: "purple", collapsed: false }],
    );
    vi.stubGlobal("browser", fake.browser);

    // Shaped exactly as planTidy would build it: github stays an untouched block
    // in `order` yet is absent from `groups`; only stackoverflow.com is new.
    const plan: TabPlan = {
    ...
    expect(result.createdGroupKeys).toEqual(["stackoverflow.com"]);
    expect(result).toEqual({
      grouped: 2,
      groupsCreated: 1,
    ...
    // github's group (42) is untouched: same 6 members, same metadata.
    expect(fake.strip.filter((tab) => tab.groupId === 42).map((tab) => tab.id)).toEqual([
      1, 2, 3, 4, 5, 6,
    ]);
    expect(fake.groupMeta.get(42)).toEqual({
      title: "github.com",
      color: "purple",
      collapsed: false,
    });

    // The freshly-created group is a distinct id carrying stackoverflow's pink,
    // never github's purple.
    const soGroupId = fake.strip.find((tab) => tab.id === 7)!.groupId;
    expect(soGroupId).not.toBe(42);
    expect(fake.groupMeta.get(soGroupId)).toEqual({
      title: "stackoverflow.com",
      color: "pink",
      collapsed: false,
    });
