...
function PopupHeader({
  ...
  return (
    <>
      ...
      <p className="facts-line num">{factsText}</p>
    </>
  );
}

function LoadingView({ isError, message }: { isError: boolean; message: string }) {
  return (
...
function EmptyView({ totalTabs }: { totalTabs: number }) {
  return (
    <div className="popup-scroll">
      <div className="empty-block">
        ...
        <span className="empty-sub">
          This window has {pluralize(totalTabs, "tab")}. Open a few more and Tab Sorter will find a
          shape worth grouping.
        </span>
      </div>
    </div>
  );
...
function SortRow({ pending, onSort }: { pending: boolean; onSort: (mode: SortMode) => void }) {
  return (
  ...
  return duplicateCount > 0 ? (
    <div>
      <div className="dedupe-row">
        ...
        <button
          className={armed ? "btn-danger-armed" : "btn-outline"}
          disabled={pending}
          onClick={onDedupeClick}
          type="button"
        >
          Close {duplicateCount} duplicates{armed ? "?" : ""}
        </button>
      </div>
      {armed ? (
...
function PatternSection({
  ...
  flags: string;
  pending: boolean;
  preview: { count: number; error: string };
  previewStale: boolean;
  matchableTabCount: number;
  presets: readonly RegexPreset[];
  moveLabel: string;
  onToggle: () => void;
  onSourceChange: (source: string) => void;
  onCaseToggle: () => void;
  onPresetApply: (preset: RegexPreset) => void;
  onExtract: () => void;
}) {
  return (
    <>
      ...
      {open ? (
        <div className="pattern-panel" id="pattern-panel">
          ...
          >
            {preview.error.length > 0
              ...
              : source.trim().length === 0
                ...
                : `Matches ${preview.count} of ${matchableTabCount} tabs`}
          </p>
          ...
          <button
            className="move-btn"
            disabled={pending || previewStale || preview.error.length > 0 || preview.count === 0}
            onClick={onExtract}
            type="button"
          >
            {moveLabel}
...
// Owns the whole copy/download feature — the selected format, the ✓ flash,
// and the sr-only announcement never concern App's reducer.
function CopySection({
  ...
  const [copyFormat, setCopyFormat] = useState<ClipboardFormat>("markdown");
  ...
  function handleDownload(format: ExportFormat) {
    ...
    void run(
      ...
      () => {
        onError("Couldn't download tabs.");
      },
    );
  }
        ...
        <div aria-label="Copy format" className="format-group" role="group">
          {COPY_FORMATS.map((option) => (
            <button
              ...
              disabled={pending}
              key={option.format}
              onClick={() => setCopyFormat(option.format)}
              type="button"
            >
...
// The non-empty main view: every section call site, moved verbatim from App.
function PopupBody({
  data,
  status,
  pending,
  ...
  regexFlags: string;
  patternOpen: boolean;
  regexPreview: { count: number; error: string };
  previewStale: boolean;
  moveLabel: string;
  ...
  onDedupeClick: () => void;
  onUndo: () => void;
  onDomainExtract: (group: DomainGroup) => void;
  onRegexExtract: () => void;
}) {
  return (
    <div className="popup-scroll">
      ...
      <HeroTidy onTidy={onTidy} pending={pending} tidyPending={tidyPending} />

      <SortRow onSort={onSort} pending={pending} />
      ...
      <DomainSection groups={data.domainGroups} onExtract={onDomainExtract} pending={pending} />

      <hr className="rule" />

      <PatternSection
        flags={regexFlags}
        matchableTabCount={data.tabs.length}
        moveLabel={moveLabel}
        onCaseToggle={() => dispatch({ type: "caseFlagToggled" })}
        onExtract={onRegexExtract}
        onPresetApply={(preset) =>
          dispatch({ type: "presetApplied", source: preset.source, flags: preset.flags })
        }
        onSourceChange={(source) => dispatch({ type: "regexSourceChanged", source })}
        onToggle={() => dispatch({ type: "patternToggled" })}
        open={patternOpen}
        pending={pending}
      ...
      <CopySection
        onBeforeAction={disarmDedupe}
        onError={(message) => dispatch({ type: "statusSet", status: { message, tone: "error" } })}
        pending={pending}
        run={run}
      />
    </div>
...
function App() {
  ...
  const { data, regexSource, regexFlags, patternOpen, dedupeArmed, status } = state;
  ...
  return (
    <main className="popup-shell">
      ...
      {data === null ? (
        <LoadingView isError={isErrorTone} message={status.message} />
      ) : isEmpty ? (
        <EmptyView totalTabs={data.totalTabs} />
      ) : (
        <PopupBody
          data={data}
          dedupeArmed={dedupeArmed}
          disarmDedupe={disarmDedupe}
          dispatch={dispatch}
          moveLabel={moveLabel}
          onDedupeClick={handleDedupeClick}
          onDomainExtract={handleDomainExtract}
          onRegexExtract={handleRegexExtract}
          onSort={handleSort}
          onTidy={handleTidy}
          onUndo={handleUndo}
